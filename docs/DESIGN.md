...
> Design notes for **legal-matter-semantic-search-semantic-search-legaltech-pytho** — a runnable python example that A Python service that routes legal matter search results into clear follow-up actions.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.vector.collection.create(...)`).
- **Feature code** — the actual task: legal matter queue.
